...
func (s *IOStreams) Execute(model tea.Model, opts ...tea.ProgramOption) (tea.Model, error) {
	if _, isWizard := model.(wizard.Model); isWizard || s.CurrentHandler == nil {
		logger.Debug("IOStreams.execute: starting new session",
			zap.String("model_type", fmt.Sprintf("%T", model)))
		...
		logger.Debug("IOStreams.execute: setting input model inside existing session",
			zap.String("model_type", fmt.Sprintf("%T", model)))
