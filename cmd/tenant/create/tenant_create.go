...
func run(opt *TenantCreateOpt, cfg *config.Config) error {
	existingTenants, err := tenant.List(cfg.Repositories.DPlatform.Value)
	if err != nil {
		return err
	}
	envs, err := environment.List(cfg.Repositories.DPlatform.Value)
	if err != nil {
		return err
	}
	...
	if err := opt.Streams.Info("Created PR link: ", result.PRUrl); err != nil {
		return err
	}
	if err := opt.Streams.Info("Tenant created successfully: ", string(t.Name)); err != nil {
		return err
	}
	return nil
}
...
func (opt *TenantCreateOpt) createParentInputSwitch(existingTenants []tenant.Tenant) userio.InputSourceSwitch[string, tenant.Name] {
	...
	for i, t := range existingTenants {
		availableTenantNames[i] = string(t.Name)
	}
	return userio.InputSourceSwitch[string, tenant.Name]{
